  ...
  then
    ...
    if [[ -z $CUSTOMER_RENTALS ]]
      ...
      if [[ ! $BIKE_ID_TO_RETURN =~ ^[0-9]+$ ]]
        ...
        RENTAL_ID=$($PSQL "SELECT rental_id FROM rentals INNER JOIN customers USING(customer_id) WHERE phone = '$PHONE_NUMBER' AND bike_id =
'$BIKE_ID_TO_RETURN' AND date_returned IS NULL")
        #if input not rented
        if [[ -z $RENTAL_ID ]]
          then
            #send to main menu
